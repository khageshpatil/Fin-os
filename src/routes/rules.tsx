...
export const Route = createFileRoute("/rules")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Nine proven financial rules, evaluated against your profile.",
      },
    ],
  }),
  component: RulesPage,
    ...
    >
      <div className="space-y-3">
        {rules.map((r, idx) => {
          ...
          const pct = Math.min(100, r.progressPct);
          return (
            <article
              key={r.id}
            ...
            >
              <div className="min-w-0">
                <div className="flex items-center gap-2.5">
                  ...
                  <span
                    className={`ml-auto inline-flex items-center gap-1.5 rounded-full border ${tone.ring} bg-background/40 px-2 py-0.5 text-[10px] font-medium ${tone.color} lg:ml-0`}
                  >
...
function Stat({ label, value, muted }: { label: string; value: string; muted?: boolean }) {
  return (
    <div className="rounded-md border border-border/60 bg-background/40 p-3">
      ...
      <div
        className={`mt-1 text-sm font-medium tabular-nums ${muted ? "text-muted-foreground" : "text-foreground"}`}
      >
        {value}
      </div>
    </div>
